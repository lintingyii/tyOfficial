...
const ITEMS = [
  ...
  },
  {
    /* Insight Nest 登入畫面的筆電 mockup。
       ...
       另起一排則是前三排一個值都不用動。這一排（它加下面那張掌機）沒湊滿
       預算（rowBudget(2)＝3.70，兩張加起來 2.50），走的是既有的「未滿排」
       路徑：維持基準高、靠左排，所以高度跟上面三排一樣，只是沒有把整排寬度
       分完，右側會留白。

       要湊滿得把這張裁到 2.70，畫面只剩螢幕中段一條，桌面、手機與光影全沒
       了；裁掉掌機那張則會切掉機身上下。兩張的內容都是整個畫面，所以寧可
       留白不填滿。 */
